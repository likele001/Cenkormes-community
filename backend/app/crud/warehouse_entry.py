...
def get_entry_by_id(db: Session, tenant_id: int, entry_id: int, with_items: bool = True) -> WarehouseEntry | None:
    ...
    return db.scalar(q.where(WarehouseEntry.id == entry_id, WarehouseEntry.tenant_id == tenant_id))
...
def list_entries(
    ...
    stmt = stmt.where(WarehouseEntry.tenant_id == tenant_id)
...
def confirm_entry(db: Session, tenant_id: int, entry: WarehouseEntry, confirmed_by: int | None = None) -> WarehouseEntry:
